...
vector_02 <- c(10,12,14,16,18,18,17,10,19,20)
...
vector_02
vector_02[-c(1,2,3)]
vector_02[-c(1:5, 8:length(vector_02))]
...
# Menor al LD(Limite de deteccion) y vacios

Cu <- c("<0.5", "<0.5", 0.8, 0.9, 0.10, 0.11, 0.12, 0.16, 0.28, 0.14)
Zn <- c(0.8, 0.5, 0.6, 0.7, 0.8, 0.9,0.14, NA, NA, NA)
...
library(tidyverse)
df$Cu_val <- as.numeric(str_remove(df$Cu, pattern = "<"))
df$Cu_val_cen <- ifelse(str_detect(df$Cu, pattern = "<"), TRUE, FALSE)

attach(df)
NADA::censummary(Cu_val, Cu_val_cen)

df %>% filter(Cu_val_cen==FALSE) %>% summarise(media = mean(Zn, na.rm=TRUE))
