...
#En esta parte trabajaremos con la data mtcars que est? por default en el software
mtcars
str(mtcars)
...
#Convertir cyl desde un num?rico a un factor variable:
...
#ECDF 
#Veremos tambi?n la funci?n de densidad acumulada emp?rica (ECDF) reporta
#para cualquier n?mero el porcentaje individual que tienen debajo de una 
#tendencia
...
#Podemos generar alfombra de n?meros
...
#Paquetes Extras:
  ...
  #un grupo de variables para analizar de las cuales realizaremos un resumen estad?stico, boxplot
  #un histograma, gr?ficos extras y curva de densidad real y ajustada y finalmente un qqplot y el an?lisis
  #de normalidad
...
#Si debemos sectar la informaci?n genere un subset de lowdose, middose, higdose correspondiente a
...
#1.0 a 2.0 cuales ser?an los subsets:
...
#Luego de revisar la informaci?n analizaremos de la forma realizada en la clase pasada:
...
#Asociarlo a un gr?fico de violin (presenta la probabilidad de densidad de Kernell de la data a diferente
#valores)
...
#Ahora lo veremos como un plot de puntos con medidas de resumen estad?stico (dotplot):
library()
...
#Produciendo sumario estad?stico personalizado:
...
#Una aplicaci?n importante para dividir data r?pidamente es dividirlo en varios paneles (usando
#facets)
...
#Podemos poner los gr?ficos m?s bonitos:
ggplot(df2,aes(x=weight,fill=sex))+
  ...
  theme_classic()
####Generaci?n de Histogramas####

##Caso 1 de an?lisis data creada df3
set.seed(1234)
df3<-data.frame(
  sex=factor(rep(c("F","M"),each=200)),
  weight=round(c(rnorm(200,mean=55,sd=5),
...
ggplot(df3,aes(x=weight))+
  geom_histogram(aes(y=..density..),color="black",fill="white",linetype=
                   "dashed")+
  geom_density(alpha=0.2,fill="#FF6666")
#Lo haremos ahora por grupo seg?n sexo:
library(dplyr)
...
####Plot Individual Mi Codigo####

#Para visualizar los gr?ficos de manera conjunta modifique el layout de la siguiente forma:
...
layout.show(2) # Muestra las 6 particiones que escog?
y<-df3
z<-df3$weight
annotation <- data.frame(
  ...
  label = c("RIC","Concentraci?n M?nima =","Q1(25%) =","Mediana(50%) =","Q3(75%) =","At?picos",
