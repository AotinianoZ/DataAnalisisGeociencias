...
caret::featurePlot(x=x, y=y, plot="density", scales=scale)

#Boxplot por Clase
x<-muestras_cu[ ,1:5]
y<-muestras_cu[ ,6]
caret::featurePlot(x=x, y=y, plot="boxplot")
...
#Library Ridges:
library(ggridges)
ggplot(muestras_cu,aes(x=Cu, y=Tipo_Roca))+  # transformar log
...
a <- ggplot(muestras_cu,aes(x=Cu,y=Au,color=Tipo_Roca))+
  geom_point(size=3.5)+
  geom_rug()
a
ggplotly(a)
